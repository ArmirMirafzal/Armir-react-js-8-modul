        ...
        <div className="col-3">
          <ul className="list-group">
            {!this.state.isLoading && (
              <li onClick={() => this.handleGenre("All")} className={`list-group-item ${this.state.genreName === "All" ? "active" : ""}`}>
                All genre
              </li>
            )}
            {genres.map((genre) => (
              <li key={genre._id} onClick={() => this.handleGenre(genre.name)} className={`list-group-item ${this.state.genreName === genre.name ? "active" : ""}`}>
                {genre.name}
              </li>
            ))}
          </ul>
